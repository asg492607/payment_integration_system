...
(function (global) {
  ...
  const PLANS = {
    starter: { label: 'Starter',  price: '₹199', duration: '30 Days' },
    pro:     { label: 'Pro',      price: '₹499', duration: '90 Days' },
    empire:  { label: 'Empire',   price: '₹999', duration: '365 Days' },
  };

  // ── State ─────────────────────────────────────────────
  let _state = {
    orderId: null, amount: null, upiLink: null,
    expiresAt: null, plan: 'pro',
  ...
  function buildModal(opts) {
    const planMeta = PLANS[opts.plan] || PLANS.pro;
  ...
  async function handleProceed() {
    ...
    try {
      const res  = await fetch(`${SERVER}/api/orders/create`, {
        ...
        body: JSON.stringify({ name, email, phone, plan: _state.plan, enterprise_id: ENT_ID }),
      });
  ...
  function showSuccess(data) {
    ...
    const plan = data?.order?.plan || _state.plan;
    const oId  = data?.order?.id || _state.orderId;
    const tId  = data?.transaction?.id || '—';

    document.getElementById('pf-act-plan').textContent  = (plan.charAt(0).toUpperCase() + plan.slice(1)) + ' Plan';
  ...
  function resetState() {
    _state = { orderId:null, amount:null, upiLink:null, expiresAt:null, plan:'pro', pollTimer:null, expiryTimer:null, opts:{}, _succeeded:false };
  }
